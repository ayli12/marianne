...
function generateMathProblem() {
  ...
  do {
      ...
      result = calculateMathProblem(mathProblem);
...
function handleCorrectAnswer() {
  ...
  let currentLevel = parseInt(document.getElementById('level').textContent);
  let newLevel = currentLevel; // Default to current level
  ...
  // Only update the level and show the pop-up if it has changed
  if (newLevel > currentLevel) {
      updateLevel(newLevel);
      displayLevelPopup(newLevel); // Display the level-specific popup
...
// Function to display level-specific popup
function displayLevelPopup(level) {
...
function triggerFireworks() {
  ...
  // Stop fireworks after some time (e.g., 5 seconds)
  setTimeout(() => {
      fireworks.stop();
  }, 5000);
}
